...
export class Sinif extends Component {
    displayName = Sinif.name

    constructor(props) {
        super(props);
        this.state = {
            data: { item1: 0, item2: [{ iD_Sinif: "", s_adi: "" }], item3: [{ iD_ders: "", ders_Adi: "" }] },
            searchText: "",
            searchTextFind: "",
            loading: true,
            pageNo: 0
    ...
    sendPageRequest() {
        ...
        fetch("api/SampleData/postSinif", {
            method: "POST",
            headers: {
                ...
                <tbody>
                    {data.item2.map((dt, index) =>
                        <tr key={index}>
                            <td>{dt.iD_Sinif}</td>
                            <td>{dt.s_adi}</td>
                            <td>{data.item3[index].ders_Adi}</td>
                        </tr>
                    )}
                </tbody>
            </table>
    ...
    render() {
        let contents = this.state.loading
            ? <p><em>Loading...</em></p>
            : Sinif.renderTable(this.state.data);

        return (
            <div>
                <div className="col-md-12">
                    <div className="row">
                        <h1 className="text-center">Sınıf Listesi</h1>
                    </div>
                    <div className="row">
                        <MDBCol md="6">
